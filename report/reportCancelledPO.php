    ...
    <div class="container">
        <div class="col-md-12"> 
            ...
            <div id="out_print">
                ...
                <h2 class="text-center d-inline-block" style="position:absolute; top:90px; right:50px;" id="title"><b>CANCELLED PURCHASE ORDER</b></h2>
                <div>
            ...
            <div class="card-body" id="out_print">
                <table class="table table-borderless" id="cancelledPO">
                    <thead>
                    <tr>
                        <th>PO Number</th>
                        <th>Remark</th>
                    ...
                    <tbody>
                    <?php 
                    $po_details = $con->query("SELECT distinct(pd.po_id) FROM po_details pd, purchaseorder po WHERE pd.po_Date BETWEEN '$from_date' AND '$to_date' AND po.po_status = 'Denied'");
                    while($row = $po_details->fetch_assoc()) {
                        $po_id = $row['po_id'];
                        $po = $con->query("SELECT * FROM purchaseorder po , po_details pd WHERE pd.po_id = po.po_id AND pd.po_id = '$po_id' AND pd.po_Date BETWEEN '$from_date' AND '$to_date' AND po.po_status = 'Denied';");
                        $poOrderTotal = 0;
                        $controlRow = true;
